...
class ThemeToggle extends Component {
  ...
  render() {
    ...
    return <button onClick={toggleTheme}>Toggle Theme Color</button>;
  }
}
...
class Header extends Component {
  ...
  addUser = (e) => {
    ...
    this.setState((preState) => {
      return { key: preState.key + 1 };
    });
    this.setState({ doc: this.state.doc + 1 });

    const db = firebase.firestore();
    db.collection('imageStock').where('level', '==', 'easy').get().then((querySnapshot) => {
  ...
  addFirebase = (e) => {
    ...
    return db.collection('imageStock').doc(this.state.doc).set({
        ...
        {(context) => {
          ...
          return (
            <div style={{ background: theme.ui, color: theme.syntax }}>
              ...
              <div>
                below: add to firebase doc "imageStock"
